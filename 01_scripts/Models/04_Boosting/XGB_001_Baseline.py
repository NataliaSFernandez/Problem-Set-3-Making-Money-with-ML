"""
XGBoost - XGB_001 BASELINE
============================
Baseline minimo: features estructurales de Properati, hiperparametros default.

  Cuanto predice XGBoost usando unicamente las variables basicas de Properati,
  sin ninguna ingenieria de features adicional?
...
6. Diagnosticos: feature importance, residuos, comparacion CV
...
Estructura de carpetas generada automaticamente al correr el script:
  ...
  ├── 02_outputs/
  │   ├── Models/04_Boosting/XGB_001/ <- graficos del modelo
  │   └── model_registry.xlsx
...
# Rutas - el script vive en 01_scripts/Models/04_Boosting/
...
# SECCION 2: FEATURES
# =============================================================================
# Solo features estructurales - baseline minimo sin OSM ni texto
...
def plot_importancia(xgb, feature_cols):
    ...
    ax.barh(list(imp.index), list(imp.values), color="#3498db", edgecolor="white")
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "notas": (
            f"Baseline: solo estructurales + es_apartamento. "
            f"Defaults XGBoost: n_est={N_ESTIMATORS}, depth={MAX_DEPTH}, "
            f"lr={LEARNING_RATE}. Sin OSM ni texto. Sesgo Delta={sesgo:+.5f}."
        ),
    }
...
def main():
    print(f"{'='*60}")
    print(f"  XGBOOST — {MODEL_ID}  (Baseline estructural)")
    print(f"{'='*60}")
