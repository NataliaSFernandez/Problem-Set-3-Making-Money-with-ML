"""
XGBoost - XGB_003 DEPTH TUNING
================================
Todas las features + tuning de max_depth y learning_rate con GridSearchCV
usando CV espacial.

  Cuanto mejora XGBoost al controlar la profundidad de cada arbol
  y la velocidad de aprendizaje?

  Grilla de busqueda:
    max_depth     en {3, 5, 7, 9}
    learning_rate en {0.01, 0.05, 0.1, 0.3}
    n_estimators  = 300  (fijo — mas arboles con lr bajo)

Pipeline
--------
1. Carga  train_final.csv / test_final.csv
2. Features: STRUCTURAL + OSM + TEXT + es_apartamento
3. GridSearchCV espacial -> mejores max_depth y learning_rate
4. CV aleatorio 5-fold  (KFold)      -> MAE_rand
5. CV espacial  5-fold  (GroupKFold) -> MAE_esp
6. Modelo final con mejores hiperparametros
7. Diagnosticos: feature importance, curva de aprendizaje, residuos, CV
...
Estructura de carpetas generada automaticamente al correr el script:
  ...
  ├── 02_outputs/
  │   ├── Models/04_Boosting/XGB_003/ <- graficos del modelo
  │   └── model_registry.xlsx
...
# n_estimators fijo — mas arboles permiten learning_rate mas bajo
N_ESTIMATORS = 300

# Grilla de busqueda sobre depth y learning_rate
PARAM_GRID = {
    "max_depth":     [3, 5, 7, 9],
...
def plot_curva_aprendizaje(xgb, X_train, y_train):
    """Curva de aprendizaje: MAE train vs validacion por numero de arboles."""
...
def generar_submission(test, y_pred_log, params):
    ...
    sub_name = (
        f"XGB_nest{N_ESTIMATORS}"
        f"_d{params.get('max_depth')}"
        f"_lr{str(params.get('learning_rate')).replace('.','')}"
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "notas": (
            f"Depth tuning: GridSearch sobre max_depth y learning_rate. "
            f"n_estimators fijo={N_ESTIMATORS}. "
...
def main():
    print(f"{'='*60}")
    print(f"  XGBOOST — {MODEL_ID}  (Depth Tuning)")
    print(f"{'='*60}")
    ...
    print("\n[4/8] Tuning con GridSearchCV espacial (depth + learning_rate)...")
