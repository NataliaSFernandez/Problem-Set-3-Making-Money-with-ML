"""
XGBoost - XGB_008 FILTRO DE TRAIN POR PRECIO M2
=================================================
Filtra el train para quedarse solo con propiedades cuyo precio por m2
esta en el mismo rango intercuartilico que las propiedades de Chapinero
que si existen en el train.

Motivacion (inspirada en PS2 — excluir dominios ausentes en test):
  En el PS2 se excluyeron las observaciones de Bogota del train porque
  el test no tenia Bogota — entrenar con un dominio ausente en test
  introduce sesgo de distribucion.

  En el PS3 el test es Chapinero. Chapinero tiene un rango de precios
  por m2 especifico — propiedades fuera de ese rango (barrios muy baratos
  o muy caros comparados con Chapinero) pueden introducir ruido.

  Criterio objetivo basado en datos:
  ───────────────────────────────────
  1. Identificar propiedades de Chapinero en el train (dist_cbd_km < umbral
     o lat/lon dentro del bounding box de Chapinero)
  2. Calcular el rango intercuartilico (IQR) de precio_m2 en Chapinero
  3. Filtrar el train para quedarse con propiedades en [Q1 - 1.5*IQR, Q3 + 1.5*IQR]

  Este criterio es estadistico y reproducible — no depende de juicios
  subjetivos sobre que barrios incluir.

Pipeline
--------
1. Carga  train_final.csv / test_final.csv
2. Calcular precio_m2 y filtrar train por rango de Chapinero
3. Features: STRUCTURAL + OSM + TEXT + lat + lon + es_apartamento
4. RandomizedSearchCV espacial (50 iter)
5. Modelo final con early stopping
6. CV aleatorio y espacial sobre el train filtrado
7. Diagnosticos: distribucion del filtro, feature importance, residuos
...
Estructura de carpetas generada automaticamente al correr el script:
  ...
  ├── 02_outputs/
  │   ├── Models/04_Boosting/XGB_008/ <- graficos del modelo
  │   └── model_registry.xlsx
...
# Bounding box de Chapinero (lat/lon aproximados)
# Usado para identificar propiedades de Chapinero en el train
CHAPINERO_LAT_MIN = 4.628
CHAPINERO_LAT_MAX = 4.668
CHAPINERO_LON_MIN = -74.075
CHAPINERO_LON_MAX = -74.045
...
# =============================================================================
# SECCION 3: CARGA, FILTRO Y PREPARACION
# =============================================================================
...
def filtrar_train_por_chapinero(train):
    """
    Filtra el train para quedarse con propiedades cuyo precio_m2
    esta en el rango IQR de las propiedades de Chapinero en el train.

    Criterio objetivo basado en datos:
    1. Identificar propiedades de Chapinero en el train por bounding box
    2. Calcular IQR de precio_m2 en Chapinero
    3. Filtrar train: solo propiedades en [Q1 - 1.5*IQR, Q3 + 1.5*IQR]
    """
    train = train.copy()
    train["precio_m2"] = train["price"] / (train["surface_total"].clip(lower=1))

    # Identificar Chapinero en el train
    chapinero_mask = (
        (train["lat"] >= CHAPINERO_LAT_MIN) & (train["lat"] <= CHAPINERO_LAT_MAX) &
        (train["lon"] >= CHAPINERO_LON_MIN) & (train["lon"] <= CHAPINERO_LON_MAX)
    )
    chapinero_train = train[chapinero_mask]

    print(f"  Propiedades de Chapinero en train: {chapinero_mask.sum():,}")

    if len(chapinero_train) < 10:
        print("  Pocas propiedades de Chapinero en train — usando percentiles del train completo")
        q1 = train["precio_m2"].quantile(0.25)
        q3 = train["precio_m2"].quantile(0.75)
    else:
        q1 = chapinero_train["precio_m2"].quantile(0.25)
        q3 = chapinero_train["precio_m2"].quantile(0.75)

    iqr   = q3 - q1
    lower = q1 - 1.5 * iqr
    upper = q3 + 1.5 * iqr

    print(f"  Rango precio_m2 Chapinero: [{lower:,.0f}, {upper:,.0f}] COP/m2")

    mask_filtro = (train["precio_m2"] >= lower) & (train["precio_m2"] <= upper)
    train_filtrado = train[mask_filtro].copy()

    n_original  = len(train)
    n_filtrado  = len(train_filtrado)
    pct_excluido = (1 - n_filtrado / n_original) * 100

    print(f"  Train original:  {n_original:,} propiedades")
    print(f"  Train filtrado:  {n_filtrado:,} propiedades")
    print(f"  Excluidas:       {n_original - n_filtrado:,} ({pct_excluido:.1f}%)")

    return train_filtrado, lower, upper, q1, q3


def plot_filtro(train_orig, train_filt, lower, upper):
    """Visualiza la distribucion de precio_m2 antes y despues del filtro."""
    pm2_orig = train_orig["price"] / train_orig["surface_total"].clip(lower=1)
    pm2_filt = train_filt["precio_m2"]

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].hist(pm2_orig.clip(upper=pm2_orig.quantile(0.99)),
                 bins=60, color="#95a5a6", edgecolor="white", linewidth=0.3)
    axes[0].axvline(lower, color="#e74c3c", lw=1.5, linestyle="--", label=f"Lower: {lower:,.0f}")
    axes[0].axvline(upper, color="#e74c3c", lw=1.5, linestyle="--", label=f"Upper: {upper:,.0f}")
    axes[0].set_title("Precio/m2 — Train completo")
    axes[0].set_xlabel("COP por m2")
    axes[0].legend(fontsize=8)

    axes[1].hist(pm2_filt.clip(upper=pm2_filt.quantile(0.99)),
                 bins=60, color="#3498db", edgecolor="white", linewidth=0.3)
    axes[1].set_title(f"Precio/m2 — Train filtrado ({len(train_filt):,} obs)")
    axes[1].set_xlabel("COP por m2")

    plt.tight_layout()
    fig.savefig(str(DIR_MODEL / "filtro_precio_m2.png"), dpi=150)
    plt.close(fig)
    print(f"  Guardado: {MODEL_ID}/filtro_precio_m2.png")
...
def cv_aleatorio(params, X, y):
    ...
    xgb_sk = XGBRegressor(**params, n_estimators=300,
...
def ajustar_con_early_stopping(X_train, y_train, grupos, params):
    """
    Entrena con early stopping usando un bloque espacial como validacion.
    Consistente con el CV espacial del tuning — no usa train_test_split
    aleatorio para evitar optimismo en el early stopping.
    """
...
def plot_importancia(model, feature_cols):
    ...
    if imp.index.str.startswith("f").all():
...
def generar_submission(test, model, feature_cols):
    ...
    sub_name = f"XGB_filtro_chapinero_earlystop_{MODEL_ID}.csv"
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
              n_features, params, best_round, n_train_filt,
              n_train_orig, lower, upper, sub_name):
    sesgo = mae_esp - mae_rand
    nueva = {
        "model_id":          MODEL_ID,
        ...
        "notas": (
            f"Filtro train por rango IQR precio_m2 de Chapinero. "
            f"Train: {n_train_orig} -> {n_train_filt} obs. "
            f"Rango: [{lower:,.0f}, {upper:,.0f}] COP/m2. "
            f"Early stopping={EARLY_STOP}, mejor ronda={best_round}. "
...
def main():
    print(f"{'='*60}")
    print(f"  XGBOOST — {MODEL_ID}  (Filtro Chapinero + Early Stopping)")
    print(f"{'='*60}")

    print("\n[1/9] Cargando datos...")
    train_orig, test = cargar_datos()
    print(f"  TRAIN: {train_orig.shape[0]:,} | TEST: {test.shape[0]:,}")

    print("\n[2/9] Filtrando train por rango precio_m2 de Chapinero...")
    train, lower, upper, q1, q3 = filtrar_train_por_chapinero(train_orig)
    ...
    print("\n[8/9] Modelo final con early stopping...")
    ...
    plot_filtro(train_orig, train, lower, upper)
    ...
    registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
              len(feature_cols), best_params, best_round,
              len(train), len(train_orig), lower, upper, sub_name)

    print(f"\n{'='*60}")
    print(f"  RESUMEN — {MODEL_ID}")
    print(f"  Train filtrado: {len(train_orig):,} -> {len(train):,} obs")
    print(f"  Rango precio_m2: [{lower:,.0f}, {upper:,.0f}] COP/m2")
    print(f"  Early stopping: mejor ronda = {best_round}")
