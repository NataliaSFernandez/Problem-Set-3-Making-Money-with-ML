"""
XGBoost - XGB_010 KNN PRECIO PONDERADO POR DISTANCIA
======================================================
Base: XGB_009 (lat/lon + KNN K=30 + early stopping espacial)
Mejora: precio ponderado por distancia en vez de mediana simple.

Justificacion economica (Rosen, 1974):
  Las propiedades mas cercanas comparten mas amenidades de vecindario
  que las propiedades lejanas. Una propiedad a 50 metros es mas
  representativa del vecindario que una a 500 metros.

  La mediana simple de XGB_009 trata a todos los vecinos igual.
  El precio ponderado por distancia da mas peso a los mas cercanos:

    peso_i = 1 / distancia_i
    precio_ponderado = sum(precio_i * peso_i) / sum(peso_i)

  Esto captura mejor la heterogeneidad interna de Chapinero —
  un barrio con variacion de precios importante segun la cuadra.

Implementacion sin leakage:
  Train: leave-one-out — para cada propiedad, precio ponderado de
  sus K vecinos mas cercanos EXCLUYENDOLA a ella misma.

  Test: precio ponderado de K vecinos del train completo.

K = 30 fijo (mismo que XGB_009 para comparacion justa).
Distancia: BallTree haversine (geodesica real).

Pipeline
--------
1. Carga  train_final.csv / test_final.csv
2. Calcular precio ponderado K=30 vecinos por distancia (LOO en train)
3. Features: STRUCTURAL + OSM + TEXT + lat + lon + precio_pond + es_apartamento
...
Estructura de carpetas generada automaticamente al correr el script:
  ...
  ├── 02_outputs/
  │   ├── Models/04_Boosting/XGB_010/ <- graficos del modelo
  │   └── model_registry.xlsx
...
EARLY_STOP   = 50
MAX_ROUNDS   = 1000
K_VECINOS    = 30   # igual que XGB_009 para comparacion justa
...
def calcular_precio_ponderado(train, test, k=K_VECINOS):
    """
    Calcula el precio ponderado por distancia de los K vecinos mas cercanos.

    La diferencia con XGB_009 (mediana simple) es que aqui cada vecino
    contribuye en proporcion inversa a su distancia:

        peso_i = 1 / distancia_i
        precio_pond = sum(log_price_i * peso_i) / sum(peso_i)

    Los vecinos mas cercanos tienen mas influencia que los lejanos.
    Esto captura mejor la heterogeneidad de precios dentro de Chapinero.

    Train (leave-one-out):
      Busca K+1 vecinos (incluyendo la propiedad misma) y excluye
      el primero (distancia=0) para evitar leakage.

    Test:
      Busca K vecinos en el train completo. Sin leakage.
    """
    train = train.copy()
    test  = test.copy()
    ...
    # Radio de la tierra en km para convertir distancias haversine
    R_TIERRA = 6371.0

    tree = BallTree(coords_train, metric="haversine")

    # ── Train: leave-one-out ponderado ───────────────────────────────────────
    distancias_tr, indices_tr = tree.query(coords_train, k=k + 1)

    precio_pond_train = np.zeros(len(train))
    for i in range(len(train)):
        # Excluir la propiedad misma (primer vecino, distancia=0)
        dist_vec  = distancias_tr[i][1:] * R_TIERRA  # km
        idx_vec   = indices_tr[i][1:]
        precios_v = log_price[idx_vec]

        # Peso inversamente proporcional a la distancia
        # Clip para evitar division por cero en propiedades identicas
        pesos = 1.0 / np.clip(dist_vec, 1e-6, None)
        precio_pond_train[i] = np.sum(precios_v * pesos) / np.sum(pesos)

    train["precio_vecinos"] = precio_pond_train

    # ── Test: vecinos ponderados del train completo ───────────────────────────
    distancias_te, indices_te = tree.query(coords_test, k=k)

    precio_pond_test = np.zeros(len(test))
    for i in range(len(test)):
        dist_vec  = distancias_te[i] * R_TIERRA
        idx_vec   = indices_te[i]
        precios_v = log_price[idx_vec]
        pesos     = 1.0 / np.clip(dist_vec, 1e-6, None)
        precio_pond_test[i] = np.sum(precios_v * pesos) / np.sum(pesos)

    test["precio_vecinos"] = precio_pond_test

    print(f"  precio_vecinos (ponderado) train: "
          f"media={precio_pond_train.mean():.3f}, std={precio_pond_train.std():.3f}")
    print(f"  precio_vecinos (ponderado) test:  "
          f"media={precio_pond_test.mean():.3f}, std={precio_pond_test.std():.3f}")
...
def plot_importancia(model, feature_cols):
    ...
    legend = [
        ...
        Patch(color="#e74c3c", label="Precio vecinos ponderado K=30"),
    ]
...
def generar_submission(test, model, feature_cols):
    ...
    sub_name = f"XGB_knn{K_VECINOS}_weighted_earlystop_{MODEL_ID}.csv"
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "features_grupos": (
            ...
            f"precio_vecinos_ponderado=1 (K={K_VECINOS} LOO haversine), es_apartamento"
        ),
        "spatial_grid":    f"{SPATIAL_GRID}x{SPATIAL_GRID}",
        "submission_file": sub_name,
        "notas": (
            f"Base XGB_009 + precio ponderado por distancia K={K_VECINOS} vecinos. "
            f"peso_i=1/dist_i (haversine km). LOO en train. "
            f"Early stopping espacial. Sesgo Delta={sesgo:+.5f}."
        ),
    }
...
def main():
    print(f"{'='*60}")
    print(f"  XGBOOST — {MODEL_ID}  (KNN ponderado K={K_VECINOS} + lat/lon)")
    print(f"{'='*60}")
    ...
    print(f"\n[2/9] Calculando precio ponderado K={K_VECINOS} vecinos (LOO)...")
    train, test = calcular_precio_ponderado(train, test, k=K_VECINOS)
    ...
    print(f"  Features: {len(feature_cols)} "
          ...
          f"geo={n_geo}, precio_vecinos_pond=1)")
