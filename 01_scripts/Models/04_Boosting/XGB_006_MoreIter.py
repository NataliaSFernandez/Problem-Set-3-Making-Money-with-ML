"""
XGBoost - XGB_006 MORE ITERATIONS
====================================
Todas las features + RandomizedSearchCV con 150 iteraciones.
Igual que XGB_004 pero con busqueda mas exhaustiva del espacio
de hiperparametros.

  XGB_004 uso 50 iteraciones y encontro buenos hiperparametros.
  XGB_006 usa 150 iteraciones sobre el mismo espacio de busqueda
  para explorar mas combinaciones y potencialmente encontrar
  una mejor configuracion.

  Principio: RandomizedSearchCV con mas iteraciones cubre mejor
  el espacio de hiperparametros sin el costo computacional de
  GridSearchCV (Bergstra & Bengio, 2012).

Pipeline
--------
1. Carga  train_final.csv / test_final.csv
2. Features: STRUCTURAL + OSM + TEXT + es_apartamento
3. RandomizedSearchCV espacial (150 iter) -> mejores hiperparametros
...
Estructura de carpetas generada automaticamente al correr el script:
  ...
  ├── 02_outputs/
  │   ├── Models/04_Boosting/XGB_006/ <- graficos del modelo
  │   └── model_registry.xlsx
...
CV_FOLDS     = 5
SPATIAL_GRID = 5
N_ITER       = 150   # 3x mas iteraciones que XGB_004
...
# Mismo espacio de busqueda que XGB_004 para comparacion justa
PARAM_DIST = {
...
def generar_submission(test, y_pred_log, params):
    ...
    sub_name = (
        f"XGB_iter{N_ITER}_nest{params.get('n_estimators')}"
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "notas": (
            f"All features. RandomizedSearchCV {N_ITER} iter (3x XGB_004). "
            f"Mismo espacio de busqueda que XGB_004 para comparacion justa. "
...
def main():
    print(f"{'='*60}")
    print(f"  XGBOOST — {MODEL_ID}  ({N_ITER} iteraciones de busqueda)")
    print(f"{'='*60}")
    ...
    print(f"\n[4/8] RandomizedSearchCV espacial ({N_ITER} iter)...")
    print("  Esto puede tardar 45-60 minutos...")
