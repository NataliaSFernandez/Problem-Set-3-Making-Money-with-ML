...
-----------------------
  ...
  (m², cuartos, baños, mes, año) con los hiperparámetros default de sklearn.
    ...
    ¿Cuánto predice el RF usando únicamente las variables básicas de Properati,
    sin ninguna ingeniería de features adicional?
 
  Hiperparámetros son los default de sklearn:
    n_estimators    = 100    (default sklearn)
    max_depth       = None   (árboles crecen hasta pureza)
    min_samples_leaf= 1      (default sklearn)
    max_features    = "sqrt" (default sklearn)
...
N_ESTIMATORS     = 100
MAX_DEPTH        = None   # sin podación, árboles crecen hasta pureza
MIN_SAMPLES_LEAF = 1
MAX_FEATURES     = "sqrt"
...
# SECCIÓN 2: FEATURES
# =============================================================================
# Solo features estructurales las del data set de properati 
# No se incluyen OSM ni texto 
...
def plot_importancia(rf, feature_cols):
    ...
    ax.barh(list(imp.index), list(imp.values), color="#2ecc71", edgecolor="white")
...
def generar_submission(test, y_pred_log):
    ...
    # Nombre con hiperparámetros para identificar la corrida en Kaggle
    depth_str = "dNone" if MAX_DEPTH is None else f"d{MAX_DEPTH}"
    mf_str    = str(MAX_FEATURES).replace(".", "")   # "sqrt" → "sqrt"
    sub_name  = (
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "notas": (
            ...
            f"Defaults sklearn: n_est={N_ESTIMATORS}, depth=None, leaf=1. "
            f"Referencia mínima sin OSM ni texto. "
            f"Sesgo Δ={sesgo:+.5f}."
        ),
    }
...
def main():
    print(f"{'='*60}")
    print(f"  RANDOM FOREST — {MODEL_ID}  (Baseline estructural)")
    print(f"{'='*60}")
