#!/usr/bin/env python3
"""
Random Forest — RF_004 MIN_SAMPLES_LEAF + MÁS ÁRBOLES
=======================================================

Cambios respecto a RF_003?
-------------------------------
  RF_004 tiene min_samples_leaf=5:
    - Cada hoja del árbol debe tener al menos 5 observaciones, RF_003 puede tener 1 obs por hoja.
    - Las predicciones de cada hoja son el promedio de ≥5 precios reales,
      no de una sola propiedad.

Hiperparámetros
---------------
    n_estimators    = 500   ← sube (más diversidad en el ensemble)
    max_depth       = 10
    min_samples_leaf= 5     ← cambio clave: hojas con ≥5 observaciones
    max_features    = "sqrt"
"""
...
# ── Hiperparámetros ───────────────────────────────────────────────────────────
N_ESTIMATORS     = 500   # ↑ más árboles = mayor estabilidad del ensemble
MAX_DEPTH        = 10
MIN_SAMPLES_LEAF = 5     # ← cambio clave: hojas suaves con ≥5 obs
MAX_FEATURES     = "sqrt"


# =============================================================================
# SECCIÓN 2: FEATURES (idénticas a RF_002 y RF_003)
# =============================================================================
...
def registrar(mae_rand, std_rand, mae_esp, std_esp, mae_train,
    ...
    nueva = {
        ...
        "notas": (
            f"min_samples_leaf=5: suaviza predicciones en zonas escasas. "
            f"n_estimators=500 para mayor estabilidad. "
            f"max_depth=10. Features completas. "
            f"Sesgo Δ={sesgo:+.5f}."
        ),
    }
...
def main():
    print(f"{'='*60}")
    print(f"  RANDOM FOREST — {MODEL_ID}  (min_samples_leaf=5, n_est=500)")
    print(f"{'='*60}")
